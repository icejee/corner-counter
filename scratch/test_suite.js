...
global.document = {
  ...
  createElement: () => ({ setAttribute: () => {}, appendChild: () => {}, removeChild: () => {}, click: () => {}, classList: { add: () => {}, remove: () => {} } }),
  getElementById: () => null,
  querySelector: () => null,
  addEventListener: () => {},
  readyState: 'complete'
...
async function runTests() {
  ...
  // Test 4: Superadmin Login & Company Deletion
  loginUser("JOESH", "@Icejee01");
  assert("Superadmin login succeeds", state.session.role === "superadmin" && state.session.username === "JOESH");

  // Test company creation & deletion
  const initialCompanyCount = state.companies.length;
  state.companies.push({
    ...
    staff: [],
    invites: [],
    subscription: { status: "active", plan: "standard", startAt: Date.now(), expiresAt: Date.now() + 86400000 }
  });
  assert("Temp company created", state.companies.length === initialCompanyCount + 1);

  // Delete temp company
  state.companies = state.companies.filter(c => c.id !== "company_temp");
  assert("Superadmin company deletion successful", state.companies.length === initialCompanyCount);

  // Test 5: Cloud Synchronization Engine
  queuePendingSync("sale", { id: "sale_offline_1", total: 15.50 });
  assert("Offline pending sync item queued", state.pendingSyncQueue.length === 1 && state.syncStatus === "offline_pending");

  // Perform cloud sync
  await syncWithCloud(true);
  assert("Cloud sync flushes pending queue and updates state to synced", appContext.state.pendingSyncQueue.length === 0 && appContext.state.syncStatus === "synced");
  assert("Last sync timestamp updated", appContext.state.lastSyncTime !== null);

  // Test 6: Superadmin Subscription Controls
  ...
  // Test 7: Cart and Category filtering
  ...
  // Test 9: Adding items & cart calculation
